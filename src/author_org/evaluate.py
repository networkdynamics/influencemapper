...
def evaluate(gold_triples, predict_triples, mode=3):
    ...
    for gold, predict in zip(gold_triples, predict_triples):
        ...
        if mode == 1:
            ...
            gold_tuples2 = [(entry[0]) for entry
                            in
                            gold]
            prediction_tuples = [(entry[0], unique_map[entry[1]], entry[2]) for entry in predict if (entry[0]) in gold_tuples2]
...
if __name__ == '__main__':
    gold = [line.strip() for line in open(
        '/Users/blodstone/Research/influencemapper/InfluenceMapper/data/valid_repaired_resolved_triples.jsonl')]
    # gold = open(
    ...
    predict = [line.strip() for line in open(
        '/Users/blodstone/Research/influencemapper/InfluenceMapper/data/valid_openai_4omini_ft_triples.jsonl')]
