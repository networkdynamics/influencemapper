...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        rows = []
        for _, author_data in dataset['author_info'].items():
            ...
            rows.append([(org, rels) for org, rels in orgs_rel.items()])
    ...
    coi_statements = [' '.join(dataset['coi_statements']) if type(dataset['coi_statements']) is list else dataset['coi_statements'] for dataset in datasets]
    outputs = []
    valid = False
    ...
    open("/Users/blodstone/Research/influencemapper/InfluenceMapper/data/author_org/valid_finetune.jsonl", "w").write('\n'.join(outputs))
    print(len(outputs))
    # client.files.create(
